...
@author: sazid
"""
#Objective: toad and backgrund image classification experiment with camera trap dataset.
...
DATADIR= "C:\\Users\\camera trap\\CNN1\\toad_background_CNN1\\DATADIR" 
test="C:\\Users\\\\camera trap\\CNN1\\toad_background_CNN1\\test" 

####################################################
# Set image category
CATEGORIES = [ "background","toad"]
test_categories=[ "background","toad"]
...
def create_training_data():
    for category in CATEGORIES:  # do background and toad
        ...
        path = os.path.join(DATADIR,category)  # create path to background and toad
        class_num = CATEGORIES.index(category)  # get the classification  (0 or a 1). 0=background, 1=toad
        print (class_num)
        for img in tqdm(os.listdir(path)):  # iterate over each image per background and toad
            try:
...
def create_test_data():
    for category in test_categories:   # do background and toad

        path = os.path.join(test,category)  # create path to background and toad
        class_num = test_categories.index(category)  # get the classification  (0 or a 1). 0=background, 1=toad
...
plt.title('model Accuracy & Loss of toad Vs background')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train_acc', 'validation_acc','train_loss', 'validation_loss'], loc='best')
plt.savefig('toad.png')
plt.show()
...
def plot_confusion_matrix(cm, classes, normalize=True, title='Confusion matrix of toad Vs background', cmap=plt.cm.Blues):
    """
...
plt.savefig('toad_conf_CNN1.png')
plt.show()
