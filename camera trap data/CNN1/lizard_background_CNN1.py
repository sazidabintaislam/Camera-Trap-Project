...
@author: sazid
"""
#Objective: lizard and backgrund image classification experiment with camera trap dataset.
...
DATADIR= "C:\\Users\\camera trap\\CNN1\\lizard_background_CNN1\\DATADIR" 
test="C:\\Users\\camera trap\\CNN1\\lizard_background_CNN1\\test" 

####################################################
# Set image category

CATEGORIES = [ "background","lizard"]
test_categories=[ "background","lizard"]
...
def create_training_data():
    for category in CATEGORIES:  # do "background" and "lizard"

        path = os.path.join(DATADIR,category)  # create path to "background" and "lizard"
        class_num = CATEGORIES.index(category)  # get the classification  (0 or a 1). 0=background and 1=lizard
        print (class_num)
        for img in tqdm(os.listdir(path)):  # iterate over each image per "background" and "lizard"
            try:
...
def create_test_data():
    for category in test_categories:  # do "background" and "lizard"

        path = os.path.join(test,category)  # create path to "background" and "lizard"
        class_num = test_categories.index(category)  # get the classification  (0 or a 1). 0=background and 1=lizard
...
plt.title('model Accuracy & Loss of lizard Vs background')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train_acc', 'validation_acc','train_loss', 'validation_loss'], loc='best')
plt.savefig('lizard_CNN1.png')
plt.show()
...
def plot_confusion_matrix(cm, classes, normalize=True, title='Confusion matrix of lizard Vs background', cmap=plt.cm.Blues):
    """
...
plt.savefig('lizard_conf_CNN1.png')
plt.show()
