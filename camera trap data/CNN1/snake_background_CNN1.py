...
@author: sazid
"""
#Objective: snake and backgrund image classification experiment with camera trap dataset.
...
DATADIR= "C:\\Users\\camera trap\\CNN1\\snake_background_CNN1\\DATADIR" 
test="C:\\Users\\camera trap\\CNN1\\snake_background_CNN1\\test"

####################################################
# Set image category

CATEGORIES = [ "background","snake"]
test_categories=[ "background","snake"]
...
def create_training_data():
    for category in CATEGORIES:  # do background and snake

        path = os.path.join(DATADIR,category)  # create path to background and snake
        class_num = CATEGORIES.index(category)  # get the classification  (0 or a 1). 0=background and 1=snake
        print (class_num)
        for img in tqdm(os.listdir(path)):  # iterate over each image per background and snake
            try:
...
def create_test_data():
    for category in test_categories:  # do background and snake

        path = os.path.join(test,category)  # create path to background and snake
        class_num = test_categories.index(category)  # get the classification  (0 or a 1). 0=background and 1=snake
...
plt.title('model Accuracy & Loss of snake Vs background')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train_acc', 'validation_acc','train_loss', 'validation_loss'], loc='best')
plt.savefig('snake_CNN1.png')
plt.show()
...
def plot_confusion_matrix(cm, classes, normalize=True, title='Confusion matrix of snake Vs background', cmap=plt.cm.Blues):
    """
...
plt.savefig('snake_conf_CNN1.png')
plt.show()
