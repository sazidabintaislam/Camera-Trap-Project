...
@author: sazid
"""
##Objective: Multiclass image classification experiment (lizard, snake, toad and background) with internet dataset.
...
DATADIR= "C:\\Users\\camera trap\\CNN1\\3Species_background_CNN1\\DATADIR" 
test="C:\\Users\\camera trap\\CNN1\\3Species_background_CNN1\\test" 

####################################################
# Set image category

CATEGORIES = [ "background","snake", "toad", "lizard"]
test_categories=[ "background","snake", "toad", "lizard"]
...
def create_training_data():
    for category in CATEGORIES:  # do background and toad

        path = os.path.join(DATADIR,category)  # create path to "background","snake", "lizard", "toad"
        class_num = CATEGORIES.index(category)  # get the classification  (0 or a 1). 0=background,1=snake, 2=lizard, 3=toad
        print (class_num)
        for img in tqdm(os.listdir(path)):  # iterate over each image per "background","snake", "lizard", "toad"
            try:
...
def create_test_data():
    for category in test_categories:   # do "background","snake", "lizard", "toad"

        path = os.path.join(test,category)  # create path to "background","snake", "lizard", "toad"
        class_num = test_categories.index(category)  # get the classification  (0 or a 1). 0=background,1=snake,2=lizard,3=toad
...
history=model.fit(X_train, y_train, batch_size=32, epochs=100, validation_split=0.2) #,callbacks=[CB]))
...
plt.title('model Accuracy & Loss of background,snake,lizard,toad')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train_acc', 'validation_acc','train_loss', 'validation_loss'], loc='best')
plt.savefig('all_four_CNN1.png')
plt.show()
...
def plot_confusion_matrix(cm, classes, normalize=True, title='Confusion matrix of background,snake,lizard,toad, cmap=plt.cm.Blues', cmap=plt.cm.Blues):
    """
...
plt.savefig('allfour_conf_CNN1.png')
plt.show()
